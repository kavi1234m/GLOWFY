...
/***********dimmer*************/
#define DIMMER_BRIGHTNESS_INCREASE   1
//#define DIMMER_BRIGHTNESS_INCREASE   2
...
#define SCENE_SAMPLE_ID 0X0010
void brightness_control_using_dimmer(uint8_t type)
{
	cmd_success_flag=1;
//	static s32 delta_level = 0; // DICMP_LEVEL_DELTA_VALUE
//	s32 delta_level = (65536/10);
	wakeup_time = clock_time();
    u16 pub_addr=0;
    u8 ele_offset=0;
    model_common_t *p_model;
    u8 model_idx = 0;
    p_model = (model_common_t *)mesh_find_ele_resource_in_model(ele_adr_primary + ele_offset, SIG_MD_SCENE_C, 1, &model_idx, 0);
	if(p_model && p_model->pub_adr){
		pub_addr = p_model->pub_adr;
	}else{
		pub_addr += ele_offset;
	...
	if(pub_addr)
	{
		...
		{
			 access_cmd_scene_recall(pub_addr, 0, SCENE_SAMPLE_ID, 0, 0);
			//access_cmd_set_delta(pub_addr, 0, (delta_level), CMD_NO_ACK, 0);
		}
		else
		{
			access_cmd_scene_store(pub_addr, 0, SCENE_SAMPLE_ID, 0);
			//access_cmd_set_delta(pub_addr, 0, (-delta_level), CMD_NO_ACK, 0);
		}
	}
}
