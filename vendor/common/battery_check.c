...
_attribute_ram_code_ int app_battery_power_check(int alram_vol_mv, int loop_flag)
{
...
//		LOG_BATTERY_CHECK_DEBUG(0,0,"battery low, loop:%d, current:%d, thres:%d", loop_flag, batt_vol_mv, alram_vol_mv);
	...
	if(blc_ll_getCurrentState() == BLS_LINK_STATE_ADV){
		...
		bls_att_pushNotifyData (CHARACTERISTIC_UUID_BATTERY_LEVEL, tbl_advData, 4+ADC_SAMPLE_NUM*2);//SERVICE_UUID_BATTERY //CHARACTERISTIC_UUID_BATTERY_LEVEL
	}
#endif
...
*****************************************************************************************/
_attribute_no_inline_ int app_battery_power_check_and_sleep_handle(int loop_flag)
{
	u16 volt=0;
#if 1 // deep
    ...
    if(loop_flag){
		...
		|| pm_is_deepPadWakeup() // 32k rc not run without PM_WAKEUP_TIMER  // 
            ...
            volt=app_battery_power_check(alarm_thres, loop_flag);  //2.0 V
            if(batt_vol_mv < 2500){
        ...
        if(analog_read(DEEP_ANA_REG0) & FLD_LOW_BATT_FLG){
            volt=app_battery_power_check(alarm_thres + 200, loop_flag);  //2.2 V
        }else{
            volt=app_battery_power_check(alarm_thres, loop_flag);  //2.0 V
        }
