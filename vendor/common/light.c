...
void mesh_global_var_init_light_sw()
{
	foreach_arr(i,light_res_sw){
		...
		g_def_trans_time_val(i) = PTS_TEST_EN ? 0 : 0;//TRANSITION_TIME_DEFAULT_VAL;
	}
}
...
{
	led_event_pending = e;
	if(e== LED_EVENT_1SEC_ON_2SEC_OFF_30S)
	{
		blink_led =1;
	}
}
...
void proc_led(void)
{
	...
	if (led_event_pending)
	{
		if(led_event_pending == LED_EVENT_1SEC_ON_2SEC_OFF_30S)
		{
			blink_led=1;
		}
...
{
	light_ev_with_sleep(3, 500*1000);	//1Hz shine for  1 second
}
#endif
