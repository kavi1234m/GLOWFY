...
#if (FEATURE_LOWPOWER_EN || SPIRIT_PRIVATE_LPN_EN)  //FEATURE_LOWPOWER_EN
...
#define RETRY_POLL_AGAIN_FLIP_FSN_EN		0	// change FSN to retry the 2nd round.
...
void friend_cmd_send_poll()
{
    ...
    fri_ship_proc_lpn.poll_time_ms = clock_time_ms();
...
{
//	LOG_USER_MSG_INFO(0, 0,"AFTER POLL");
//	ENABLE_SUSPEND_MASK;
...
////		LOG_MSG_LIB(TL_LOG_FRIEND, 0, 0,"LPN POLL RETRY");
...
//            	LOG_USER_MSG_INFO(0, 0,"POLL INTERVAL 1");
//            	if(!subsc_list_retry.tick){
//					if(mesh_lpn_subsc_pending.op){
//						LOG_USER_MSG_INFO(0, 0,"POLL INTERVAL 2");
//						mesh_lpn_sleep_prepare(CMD_ST_POLL_MD);
//					}
//					else{
//						LOG_USER_MSG_INFO(0, 0,"POLL INTERVAL 3");
//						mesh_friend_ship_start_poll();
//					}
//				}
//			}
...
void lpn_set_poll_ready()
{
    ...
    fri_ship_proc_lpn.poll_time_ms = fri_ship_proc_lpn.poll_time_ms - BIT(31);
}
...
void mesh_friend_ship_start_poll()
{
    if(is_in_mesh_friend_st_lpn()){
		...
		#else
		fri_ship_proc_lpn.poll_retry = PTS_TEST_EN ? 20 : (FRI_POLL_RETRY_MAX); //(FRI_POLL_RETRY_MAX + 1)
		#endif
    }
}
