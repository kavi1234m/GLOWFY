...
cbk_fun scene_id_callback;
uint8_t scene_id_buffer[35];
uint8_t scene_cbk_flag =0;
...
int mesh_cmd_sig_scene_reg_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	if(scene_cbk_flag)
	{
		memcpy(scene_id_buffer,par,par_len);
		scene_id_callback(scene_id_buffer,par_len);
	}
	par[par_len] = 0;
    int err = 0;
    if(cb_par->model){  // model may be Null for status message
...
/***********************************************************
  ...
  * @retval None
  ***********************************************************/
void scene_id_get_callback_register(void *(callback_fun))
{
	scene_cbk_flag = 1;
	scene_id_callback = callback_fun;
}
