...
STATIC_ASSERT(sizeof(mesh_scan_rsp_t) >= 31);
#endif
void mesh_scan_rsp_add_local_name_(u8 *p_name, u32 len_max)
{
	memcpy(p_name,DEVICE_NAME, min(len_max, DEVICE_LEN - 1));
}
...
//	tbl_scanRsp.type = GAP_ADTYPE_LOCAL_NAME_COMPLETE;
	...
	mesh_scan_rsp_add_local_name_(tbl_scanRsp.name, sizeof(tbl_scanRsp.name));
	#if 0
