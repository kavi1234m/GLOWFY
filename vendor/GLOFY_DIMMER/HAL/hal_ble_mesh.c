...
//	static s32 delta_level = 0; // DICMP_LEVEL_DELTA_VALUE
    ...
    model_common_t *p_model;
    u8 model_idx = 0;
    p_model = (model_common_t *)mesh_find_ele_resource_in_model(ele_adr_primary + ele_offset, SIG_MD_G_LEVEL_C, 1, &model_idx, 0);
	if(p_model && p_model->pub_adr){
		pub_addr = p_model->pub_adr;
	}else{
		pub_addr += ele_offset;
	...
	if(pub_addr)
	{
		...
		{
//			access_cmd_set_level_move(pub_addr,0,(delta_level),CMD_NO_ACK,0);
